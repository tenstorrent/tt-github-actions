...
| `run-result` | no | `""` | Aggregate result of the jobs this report covers. Accepts `success`, `failure`, `cancelled`, `skipped`. Suppresses INFRA_FAILURE synthesis on `cancelled`/`skipped`, and states those in the report header so a partial report is not read as complete. **Must be passed together with `expected-jobs`.** |
