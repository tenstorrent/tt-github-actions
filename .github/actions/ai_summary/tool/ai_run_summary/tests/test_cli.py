...
class TestMain:
    ...
    def test_model_none_skips_llm(self, tmp_path):
        ...
        with patch("sys.argv", ["ai-run-summary", "--config", config_str]):
            with patch.dict(os.environ, {"GITHUB_RUN_ID": "12345"}, clear=False):
                main()
        report = tmp_path / "ai_run_summary_r12345.md"
    ...
    def test_run_metadata_from_env(self, tmp_path):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("sys.argv", ["ai-run-summary", "--config", config_str]):
                main()
        report = tmp_path / "ai_run_summary_r77777.md"
    ...
    def test_relative_dirs_resolve_against_workspace(self, tmp_path):
        ...
        with patch("sys.argv", ["ai-run-summary", "--config", config_str]):
            with patch.dict(os.environ, {"GITHUB_RUN_ID": "99"}, clear=False):
                main()
        assert (tmp_path / "out" / "ai_run_summary_r99.md").exists()
        # .html is written alongside .md and consumed by Slack screenshot.
        assert (tmp_path / "out" / "ai_run_summary_r99.html").exists()
    ...
    def test_writes_json_alongside_md_and_html(self, tmp_path):
        ...
        with patch("sys.argv", ["ai-run-summary", "--config", config_str]):
            with patch.dict(os.environ, {"GITHUB_RUN_ID": "12345"}, clear=False):
                main()
        json_file = tmp_path / "ai_run_summary_r12345.json"
    ...
    def test_env_vars_in_workspace_expanded(self, tmp_path, monkeypatch):
        ...
        with patch("sys.argv", ["ai-run-summary", "--config", config_str]):
            with patch.dict(os.environ, {"GITHUB_RUN_ID": "55"}, clear=False):
                main()
        assert (tmp_path / "out" / "ai_run_summary_r55.md").exists()
...
class TestAttemptScopedRunReport:
    """Each attempt's report must survive alongside the others.

    A partial re-run deletes nothing and a full re-run does not reliably purge,
    so several attempts coexist and consumers pick the highest.
    """

    def _run(self, tmp_path, env):
        TestMain._write_summaries(TestMain(), tmp_path)
        config_str = json.dumps({"model": "none", "workspace": str(tmp_path), "input_dir": ".", "output_dir": "."})
        with patch("sys.argv", ["ai-run-summary", "--config", config_str]):
            with patch.dict(os.environ, env, clear=False):
                main()

    def test_attempt_is_in_the_stem(self, tmp_path):
        self._run(tmp_path, {"GITHUB_RUN_ID": "12345", "GITHUB_RUN_ATTEMPT": "2"})
        for suffix in (".md", ".html", ".json"):
            assert (tmp_path / f"ai_run_summary_r12345_a2{suffix}").exists()

    def test_reports_from_two_attempts_coexist(self, tmp_path):
        self._run(tmp_path, {"GITHUB_RUN_ID": "12345", "GITHUB_RUN_ATTEMPT": "1"})
        self._run(tmp_path, {"GITHUB_RUN_ID": "12345", "GITHUB_RUN_ATTEMPT": "2"})
        assert sorted(p.name for p in tmp_path.glob("ai_run_summary_*.md")) == [
            "ai_run_summary_r12345_a1.md",
            "ai_run_summary_r12345_a2.md",
        ]

    def test_attempt_recorded_in_the_json(self, tmp_path):
        self._run(tmp_path, {"GITHUB_RUN_ID": "12345", "GITHUB_RUN_ATTEMPT": "3"})
        data = json.loads((tmp_path / "ai_run_summary_r12345_a3.json").read_text())
        assert data["run_attempt"] == 3

    def test_no_attempt_segment_outside_ci(self, tmp_path):
        self._run(tmp_path, {"GITHUB_RUN_ID": "12345"})
        assert (tmp_path / "ai_run_summary_r12345.md").exists()
